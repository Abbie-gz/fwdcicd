...
```
dbx execute --cluster-name=<name of interactive cluster> --job=fwdcicd-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --job=fwdcicd-sample-integration-test --files-only
...
- To trigger the release pipeline, get the current version from the `fwdcicd/__init__.py` file and tag the current code version:
```
